...
class MRIPreprocessorPhase2:
    ...
    def skull_stripping(self, nifti_files):
        ...
        for nifti_file in tqdm(nifti_files, desc="Skull stripping"):
            try:
                ...
                # Try HD-BET first via ensemble (Deep Learning + Classical)
                if not is_2d:
                    print("   🧠 Using HD-BET ensemble brain extraction...")
                    brain_mask = self._ensemble_skull_strip(nifti_file, data, img.affine)
                
                # Fallback to classical if ensemble failed or if 2D
                if brain_mask is None:
                    if is_2d:
                        print("   Using enhanced classical brain extraction (2D mode)...")
                    else:
                        print("   ⚠️  HD-BET failed or skipped, falling back to classical extraction...")
    ...
    def _try_hdbet(self, nifti_file, mode='default'):
        ...
        try:
            ...
            if not hdbet:
                print("   ⚠️  HD-BET executable not found in path")
                return None
