...
def _ensure_ml_libs():
    ...
    # Pre-emptively handle memory fragmentation on limited GPUs
...
class DetectionViewSet(viewsets.ModelViewSet):
    ...
    def run_detection(self, request, pk=None):
        """
        Queue AI detection for an uploaded MRI (doctors only).

        Returns 202 immediately with a task id; preprocessing + inference
        runs in the django-q2 worker (calls Modal for GPU preprocessing,
        then runs the ResNet on the HF Space). The frontend polls the
        DetectionResult detail endpoint until status == 'completed' or
        'failed'.
        """
        ...
        if detection_result.status == 'processing':
            return Response(
                {
        ...
        detection_result.error_message = None
        detection_result.save(update_fields=['status', 'doctor', 'error_message'])

        from django_q.tasks import async_task
        task_id = async_task(
            'detection.tasks.run_detection_task',
            detection_result.id,
            detection_result.model_type or 'binary',
        )
    ...
    def upload_and_detect(self, request):
        ...
        try:
            ...
            # Queue async preprocessing + inference. The detail endpoint
            # (GET /api/detection/<id>/) is what the frontend polls for the
            # final result.
            from django_q.tasks import async_task
            task_id = async_task(
                'detection.tasks.run_detection_task',
            ...
            return Response(
                {
                    'message': 'Upload accepted; detection queued.',
                    'detection_id': detection_result.detection_id,
            ...
            logger.exception('upload_and_detect failed')
            return Response(
                {'error': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def explainability_slices(self, request, pk=None):
        ...
        try:
            ...
            for idx in indices:
                ...
                # Release GPU memory after each slice to prevent OOM
                del gradcam_result
                if device.type == 'cuda':
