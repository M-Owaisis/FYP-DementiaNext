...
export default function DoctorDashboardPage() {
  ...
  //
  // Backend now queues a background job (django-q2) that calls Modal for the
  // 2–4 min GPU preprocessing step. We POST to start it, then poll the
  // detail endpoint until DetectionResult.status flips to completed/failed.
  const handleRunDetection = async (detectionId: number) => {
    ...
    try {
      ...
      const queueResp = await fetch(
        `${API_URL}/detection/detections/${detectionId}/run_detection/`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!queueResp.ok && queueResp.status !== 202) {
        const data = await queueResp.json().catch(() => ({}))
        alert(data.error || 'Failed to queue detection')
        return
      }

      // Refresh once so the row shows "processing" while we wait.
      fetchDetections()

      // Poll: 4-second interval, 12-minute hard cap. Modal preprocessing is
      // ~2–4 min on a warm container, plus cold-start padding.
      ...
      while (Date.now() - startedAt < TIMEOUT_MS) {
        ...
        const statusResp = await fetch(
          `${API_URL}/detection/detections/${detectionId}/`,
          { headers: { Authorization: `Bearer ${token}` } },
        )
        ...
        if (det.status === 'failed') {
          fetchDetections()
          alert(`Detection failed: ${det.error_message || 'unknown error'}`)
          return
        }
      }

      // Timed out — leave the row as 'processing' so a refresh will pick it up.
      fetchDetections()
      alert(
        'Detection is still running. It may have hit a Modal cold start; refresh the dashboard in a minute to see the result.',
      )
    } catch (error) {
      console.error('Error running detection:', error)
      alert('Network error while running detection.')
    } finally {
      setActionLoading(null)
    }
  }
