...
function DetectionInner() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // The backend now returns 202 with { detection_id, id, status: 'processing' }.
      // Heavy preprocessing runs on Modal (2–4 min). Poll the detail endpoint
      // until the worker writes the final result.
      const queued = await response.json()
      const detectionPk = queued.id
      if (!detectionPk) {
        throw new Error('Backend did not return a detection id.')
      }
      ...
      while (Date.now() - startedAt < TIMEOUT_MS) {
        ...
        const statusResp = await fetch(
          `${API_BASE_URL}/api/detection/detections/${detectionPk}/`,
          { headers: { Authorization: `Bearer ${token}` } },
        )
        if (!statusResp.ok) continue
        const det = await statusResp.json()
        if (det.status === 'completed') {
          result = det
          break
        }
        if (det.status === 'failed') {
          throw new Error(det.error_message || 'Detection failed during processing')
        }
      }

      if (!result) {
        throw new Error(
          'Detection is still running. Refresh in a minute or check the dashboard — the worker will save the result.',
        )
      }

      const modelDisplayName = selectedModel === 'subtype'
        ? 'Subtype Classifier (4-class ResNet-34)'
        : 'Binary Dementia Detector (ResNet-34)'

      setDetectionResult({
        modelType: modelDisplayName,
        modelTypeKey: selectedModel,
        predicted_class: result.predicted_class,
        classification: result.predicted_class_display || CLASS_LABELS[result.predicted_class] || result.predicted_class,
  ...
  const fetchSingleDetection = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHasResults(true)
        if (result.patient) setSelectedPatientId(result.patient.toString())
      } else {
  ...
  const handleDownloadReport = () => {
    ...
    const formatProbabilities = () => {
      ...
      return Object.entries(detectionResult.probabilities)
        .map(([cls, prob]: [string, any]) => `${CLASS_LABELS[cls] || cls}: ${(prob * 100).toFixed(2)}%`)
        .join('\n')
    }
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  ...
                  <div className="flex gap-6 text-sm text-gray-600">
                    <div>
                      ...
                      <p className="font-medium">{detectionResult.processingTime}s</p>
                    </div>
                    <div>
